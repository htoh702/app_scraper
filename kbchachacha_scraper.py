...
class UnifiedReviewMonitor:
    def __init__(self, ios_app_id: str = None, android_package_name: str = None,
        ...
        self.resorce_dir = Path("kbchachacha")
    ...
    def save_unified_reviews(self, new_reviews: Dict[str, List[Dict]]) -> None:
        ...
        if all_new_reviews:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = self.data_dir / f"kbchachacha_reviews_{timestamp}.csv"
            df.to_csv(filename, index=False, encoding="utf-8-sig")
...
def main():
    ...
    # 앱 설정
    IOS_APP_ID = "1112366886"  # iOS
    ANDROID_PACKAGE_NAME = "kr.co.kbc.cha.android"  # Android
